...
class order_parameter(Analysis):
    ...
    def __init__(self,List,filename,plane,n_bins,fullblock=0):
        ...
        if self.plane == "x":
            ...
            self.hist_bins=[self.List.System.c_DataFrame.c_BoxSize[0]+self.dr*i for i in range(n_bins+1)]
            self.corr_vector=np.array([1,0,0])
        elif self.plane == "y":
            self.MaxDistance=self.List.System.c_AbsBoxSize[1]
            self.dr=self.MaxDistance/self.n_bins
            self.hist_bins=[self.List.System.c_DataFrame.c_BoxSize[2]+self.dr*i for i in range(n_bins+1)]
            self.corr_vector=np.array([0,1,0])
        elif self.plane == "z":
            self.MaxDistance=self.List.System.c_AbsBoxSize[2]
            self.dr=self.MaxDistance/self.n_bins
            self.hist_bins=[self.List.System.c_DataFrame.c_BoxSize[4]+self.dr*i for i in range(n_bins+1)]
    ...
    def list_statickernel(self,block,current_trajectry):
        ...
        for index_bin in range(self.n_bins):
            ...
            if self.plane == "x":
                ...
                bin_trj=current_trajectry[(self.List.wrap_pos.loc[block*self.List.System.c_BlockSize].values[:,1]>start_bin) & (self.List.wrap_pos.loc[block*self.List.System.c_BlockSize].values[:,1]<end_bin)]
                tempcorrelation=np.sum(np.multiply(bin_trj,self.corr_vector),axis=1)/(np.linalg.norm(bin_trj,axis=1))
                temporientationalcorrelation=np.sum(math_tool.legendre_polynomial(np.sum(np.multiply(math_tool.unit_vector(bin_trj),self.corr_vector),axis=1),2))/bin_trj.shape[0]
            elif self.plane == "z":
                bin_trj=current_trajectry[(self.List.wrap_pos.loc[block*self.List.System.c_BlockSize].values[:,2]>start_bin) & (self.List.wrap_pos.loc[block*self.List.System.c_BlockSize].values[:,2]<end_bin)]
                tempcorrelation=np.sum(np.multiply(bin_trj,self.corr_vector),axis=1)/(np.linalg.norm(bin_trj,axis=1))
                temporientationalcorrelation=np.sum(math_tool.legendre_polynomial(np.sum(np.multiply(math_tool.unit_vector(bin_trj),self.corr_vector),axis=1),2))/bin_trj.shape[0]
            else:
