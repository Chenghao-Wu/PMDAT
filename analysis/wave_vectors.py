...
class WaveVector(object):
    def __init__(self,System,plane,MaxLengthScale):
        ...
        if MaxLengthScale<0:
            ...
            if plane=="xy":
                if self.System.c_AbsBoxSize[0]<self.System.c_AbsBoxSize[1]:
                    min_size=self.System.c_AbsBoxSize[0]
                else:
                    min_size=self.System.c_AbsBoxSize[1]
            elif plane == "xz":
                if self.System.c_AbsBoxSize[0]<self.System.c_AbsBoxSize[2]:
                    min_size=self.System.c_AbsBoxSize[0]
                else:
                    min_size=self.System.c_AbsBoxSize[2]
            elif plane == "yz":
                if self.System.c_AbsBoxSize[1]<self.System.c_AbsBoxSize[2]:
                    min_size=self.System.c_AbsBoxSize[1]
                else:
                    min_size=self.System.c_AbsBoxSize[2]
            elif plane == "xyz":
                min_size=min(self.System.c_AbsBoxSize[0],self.System.c_AbsBoxSize[1],self.System.c_AbsBoxSize[2])
            elif plane == "x":
                min_size=self.System.c_AbsBoxSize[0]
            elif plane == "y":
                min_size=self.System.c_AbsBoxSize[1]
            ...
            if MaxLengthScale > min(self.System.c_AbsBoxSize[0],self.System.c_AbsBoxSize[1],self.System.c_AbsBoxSize[2]):
    ...
    def read_vectors_1d(self,plane):
        for wavenumberii in range(self.NumberWaveNumbers):
            ...
            filename="./qvectors/qvectors3d/qvector"+str(buff)
    ...
    def read_vectors_3d(self):
        for wavenumberii in range(self.NumberWaveNumbers):
            buff=".%03d" % (wavenumberii+2)
            filename="./analysis/qvectors/qvectors3d/qvector"+str(buff)
            qvector_file = Path(filename)
            if qvector_file.is_file():
                vector=pd.read_csv(filename,header=None,sep="\s+")
                x_vector=vector.loc[:,0]
